...
await clientService.CreateWithLogAsync(auctionItemUser.Username, auctionItemUser.Password, xmppServer, new MessageListener());
...
{
    ClientUser? auctionItemUser = configuration.GetSection($"xmppSettings:auction-item-54321").Get<ClientUser>();
    if (auctionItemUser?.Username == null)
    {
        Console.WriteLine("User details not for in test settings section 'xmppSettings:auction-item-54321'.");
...
{
    string? recipient = configuration.GetSection($"xmppSettings:sniper:Username").Get<string>();
    if (recipient == null)
    {
        Console.WriteLine("Server details not for in test settings section 'xmppSettings:sniper:Username'.");
